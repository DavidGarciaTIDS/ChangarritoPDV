...
namespace LibPDV.Users
{
    public class Users : CRUD
    {
        ...
        public List<List<object>> Read(List<string> fields, SearchCollection searching)
        {
            List<SearchCollection> search = new List<SearchCollection>();
            search.Add(searching);

            List<List<object>> res = base.Read(fields, search);
            ...
            foreach (List<object> lista in res)
            {
                Console.WriteLine("-----------------");
